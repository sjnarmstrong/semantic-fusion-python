...
public:
  ...
  bool Init(const std::string& model_path, const std::string& weights, bool cpu_only=false);
    std::shared_ptr<caffe::Blob<float> > ProcessFrame(
            const ImagePtr rgb, const DepthPtr depth,
            const int height, const int width);
    std::shared_ptr<caffe::Blob<float> > ProcessFrameNumpy(
            ImagePtr rgb_arr, int n_rgb, DepthPtr depth_arr, int n_depth,
            const int height, const int width){
        return ProcessFrame(rgb_arr, depth_arr, height, width);
    }
