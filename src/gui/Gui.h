...
public:
  ...
  void preCall();
  void renderMap(ElasticFusionInterface& map);
  void renderMap(const std::unique_ptr<ElasticFusionInterface>& map);
  void postCall();
  void displayArgMaxClassColouring(const std::string & id, int channels, const float threshold, ElasticFusionInterface& map, SemanticFusionInterface& sFusion);
  ...
  void displayImg(const std::string & id, ElasticFusionInterface& map);
