...
void Gui::renderMap(ElasticFusionInterface& map) {
...
void Gui::displayArgMaxClassColouring(const std::string & id, int channels, const float threshold, ElasticFusionInterface& map, SemanticFusionInterface& sFusion){
    displayArgMaxClassColouring(id, sFusion.get_rendered_probability()->mutable_gpu_data(), channels,
            sFusion.get_class_max_gpu()->gpu_data(), sFusion.max_num_components(), map.GetSurfelIdsGpu(), threshold);
}
...
void Gui::displayImg(const std::string & id, ElasticFusionInterface& map) {
    displayImg(id, map.getRawImageTexture());
//    displayImg(id, map.getRawDepthTexture());
}
