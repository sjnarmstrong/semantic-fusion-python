...
class LogReader
{
    public:
        ...
        int getDepthSize() {return depthSize;}
        long getTimestamp() {return timestamp;}

        void updateRGBVector(unsigned char* out_arr, int n)
        {
            assert ( n==imageSize );
            memcpy ( out_arr, rgb, n );
        }

        void updateDepthVector(unsigned short* out_arr, int n, bool use_filled=false)
        {
            assert ( n==depthSize/ sizeof(unsigned short) );
            if (use_filled){
                memcpy ( out_arr, depthfilled, depthSize );
            } else {
                memcpy ( out_arr, depth, depthSize );
            }
        }
